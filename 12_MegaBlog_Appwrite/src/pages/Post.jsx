...
export default function Post() {
  ...
  const deletePost = () => {
    postService.deletePost(post.$id).then((status) => {
      if (status) {
        fileService.deleteFileById(post.featuredImage);
        navigate("/");
      }
    });
  };
          ...
          {isAuthor && (
            <div className="absolute right-6 top-6">
              <Link to={`/edit-post/${post.$id}`}>
                <Button bgColor="bg-green-500" className="mr-3 cursor-pointer px-6 active:scale-95">
                  Edit
                </Button>
              </Link>
              <Button bgColor="bg-red-500" onClick={deletePost} className="cursor-pointer px-6 active:scale-95">
                Delete
              </Button>
            </div>
          )}
